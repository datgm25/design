...
- 卒業制作を想定した企画の仕上げと共有
  ...
  - 14週目までに、次のものを用意する
    ...
    - 仮ステージ案
    - モックアップ画面
    - 仕様書([仕様書テンプレート](https://docs.google.com/spreadsheets/d/1dSyVz3NBG7-zUDoey8ss6yPZwC6486fVwh4LGlNNvPM/))
      - [仕様書例：オープン版Voxelorer Bird仕様書](https://docs.google.com/spreadsheets/d/1z6rZWF8qitYePpJ-e99rQprVXpgaBVB05CYN53RaDE0/)
- ヒント
  - 企画書から始めるのが難しい場合は、先に試作して、それを企画書に書き戻してもよい
  - モチーフを守るために、ゲームシステムが制限されるのを避ける。ゲームシステムを優先して、モチーフはまげても構わないし、ゲームシステムとモチーフが合わなくなったら、最適なモチーフを探しなおす
  - モチーフ最優先の場合は、ノベルを検討する
- [企画概要書の講評](https://docs.google.com/document/d/1wWSeZ_YeR18M3_cSwzRXtVij9HwtDh4Q9vAPMX-YzRY/)
...
- オギハラ
  - [ ] 連絡先などを追加
  - [ ] 取り組んでいるテーマの箇条書きの「-」の後ろに半角スペースを入れて、箇条書きにする
  - [ ] ポートフォリオの作品リストに、スクリーンショットのサイズ調整
  - [ ] 開発期間と開発環境を加える
  - [ ] unityroomなどへ公開していたら、そこへのリンク
- キシノ
  - [x] ノベルをWeb用にビルドして、[ノベルゲームコレクション](https://novelgame.jp/)などに公開して、リンクを張る
  - [x] prac.mdを開いて、文字エンコードをUTF-8(BOMなし)にする
  - [x] README.mdの改行が正しくできておらず、見出しが埋もれているのを修正する
  - [x] README.mdの文字エンコードをUTF-8(BOMなし)にする
  - [ ] ノベルゲームコレクションへのリンクを貼る
- クレバヤシ
  - [x] ポートフォリオの作品リストに、スクリーンショット、ゲーム概要など、必要な情報を追加する
  - [ ] 開発期間と開発環境を加える
- ナガヤマ
  - [x] ポートフォリオを作成して、URLを共有
  - [x] ポートフォリオを作成
  - [ ] 見本に従って、書式を揃える
  - [ ] 開発期間と開発環境を加える
- ニカイドウ OK
...
- ミヤジマ OK
- ヤマダ
  - [x] ポートフォリオに連絡先（学校で作成したgmailアドレス)を追加
  - [ ] ポートフォリオの作品リストに、開発期間と開発環境を加える
  - [ ] unityroomなどへ公開していたら、そこへのリンク
...
- 仕様書の作成
  ...
  - 提出
    - 仕様書の右上の共有をクリック
    - 制限付きを、リンクを知っている全員に変更
    - リンクをコピーを選択
    - [こちら](https://docs.google.com/forms/d/e/1FAIpQLSfXGJvYbiwt1qQLB5GhHJJFp_NxH939IvLUvGuzuWzW6WgUvQ/viewform?usp=dialog)を開いて、氏名とURLを送信する
...
- DATフェスタ版のビルドデータ、インストカードのデータ、プロジェクトの提出
  ...
  - ティラノ
    - プロジェクトフォルダーをZIP圧縮する
    - `Z:\2025\student\gp1\DATフェスタ`フォルダーを開いて、新しいフォルダーを作成して、フォルダー名を作品名に変更
